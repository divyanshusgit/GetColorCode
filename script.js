...
colorName.addEventListener('keyup', generateColor);
colorName.addEventListener('keydown', blackIt);
function generateColor(e) {
    if (alphabetArray.includes(e.key) || e.key=='Backspace') {
        clearTimeout(timeOutHolder);
        timeOutHolder = setTimeout(() => {
            let color = e.target.value
            coloredSquare.style.backgroundColor = color;
            console.log(rgbColorMap[color]);
            console.log(hexColorMap[color]);
            console.log(hslColorMap[color]);
        }, 500);
    }
    else {
        e.preventDefault();
...
function blackIt(e) {
    if (alphabetArray.includes(e.key) || e.key=='Backspace') {
        coloredSquare.style.backgroundColor = 'white';
    }
    else {
        e.preventDefault();
    };
